...
if(isset($_POST['submit']))
{
  ...
  if(!empty($_POST["done_canopy"] || $_POST["done_pole"] || $_POST["done_seat"] || $_POST["done_motor"] || $_POST["done_crack"] || $_POST["done_leak"]))
  {
    $new_id = $_POST ["new_id"];
    $new_damage = 0;
    $dam_id = $_POST['dam_id'];
    ...
    if(!empty($_POST["done_canopy"]))
    {
      ...
      $stmt1 = $conn->prepare("UPDATE `damage_electric` SET dam_status_canopy=?, done_date_canopy=?, done_price_canopy=? WHERE dam_elc_id=?");
    ...
    if(!empty($_POST["done_pole"]))
    {
      ...
      $stmt2 = $conn->prepare("UPDATE `damage_electric` SET dam_status_pole=?, done_date_pole=?, done_price_pole=? WHERE dam_elc_id=?");
    ...
    if(!empty($_POST["done_seat"]))
    {
      ...
      $stmt3 = $conn->prepare("UPDATE `damage_electric` SET dam_status_seat=?, done_date_seat=?, done_price_seat=? WHERE dam_elc_id=?");
      $stmt3->bind_param("ssss", $dam_status, $done_date, $done_seat, $dam_id);
      $stmt3->execute();
    }

    if(!empty($_POST["done_motor"]))
    {
      $done_motor = $_POST["done_motor"];
      $stmt4 = $conn->prepare("UPDATE `damage_electric` SET dam_status_motor=?, done_date_motor=?, done_price_motor=? WHERE dam_elc_id=?");
      $stmt4->bind_param("ssss", $dam_status, $done_date, $done_motor, $dam_id);
      $stmt4->execute();
    }
    ...
    if(!empty($_POST["done_crack"]))
    {
      ...
      $stmt5 = $conn->prepare("UPDATE `damage_electric` SET dam_status_crack=?, done_date_crack=?, done_price_crack=? WHERE dam_elc_id=?");
    ...
    if(!empty($_POST["done_leak"]))
    {
      ...
      $stmt6 = $conn->prepare("UPDATE `damage_electric` SET dam_status_leak=?, done_date_leak=?, done_price_leak=? WHERE dam_elc_id=?");
      $stmt6->bind_param("ssss", $dam_status, $done_date, $done_leak, $dam_id);
      $stmt6->execute();
    }

    $stmt7 = $conn->prepare("SELECT * FROM `damage_electric` WHERE dam_elc_id=? ");
    $stmt7->bind_param("s", $dam_id);
    $stmt7->execute();
    $result7 = $stmt7->get_result();
    $row7 = $result7->fetch_assoc();

    $dam_elc_date = $row7 ['dam_elc_date'];

    $status_canopy = $row7 ['dam_status_canopy'];
    $status_pole = $row7 ['dam_status_pole'];
    $status_seat = $row7 ['dam_status_seat'];
    $status_motor = $row7 ['dam_status_motor'];
    ...
    $price_motor = $row7 ['done_price_motor'];
    $price_crack = $row7 ['done_price_crack'];
    $price_leak = $row7 ['done_price_leak'];

    $done_price = ($price_canopy + $price_pole + $price_seat + $price_motor + $price_crack + $price_leak);

    if ($status_canopy==0 && $status_pole==0 && $status_seat==0 && $status_motor==0 && $status_crack==0 && $status_leak==0) {
      ...
      $facLimit= ($fac_limit+ 1);
        ...
        if($stmt8){
          ...
          $stmt9 = $conn->prepare("UPDATE `damage_electric` SET rep_elc_status=?, done_elc_price=?, done_elc_date=?, done_elc_status=? WHERE dam_elc_id=?");
          ...
          $stmt12->bind_param("ssss", $dam_elc_date, $done_date, $done_price, $fac_id);
          $stmt12->execute();

          if($stmt9!=''&&$stmt10!=''&&$stmt11!=''&&$stmt12!=''){
            echo "<script> alert('Electric Boat is Done Repair!')</script>";
            echo "<script>window.location.href='maintenance_electric_repair_list.php';</script>";
          }
          else{
            echo "<script> alert('Electric Boat is Failed Repair!')</script>";
            echo "<script>window.location.href='maintenance_electric_repair_list.php';</script>";
          }
        }
      } else {
