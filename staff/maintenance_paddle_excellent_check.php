<?php
  ...
  if(isset($_POST['checked']))
  {
    ...
    if(!empty($_POST["dam_pad_canopy"]))
    {
      foreach($_POST["dam_pad_canopy"] as $a)
      {
        ...
        $stmt1 = $conn->prepare("SELECT * FROM `damage_paddle` WHERE new_id=? AND dam_pad_date=?");
        ...
        if($row1){
          $stmt2 = $conn->prepare("UPDATE `damage_paddle` SET dam_pad_canopy=?, dam_status_canopy=? WHERE new_id=? AND dam_pad_date=?");
          ...
          if($row3){
            $stmt4 = $conn->prepare("INSERT INTO `damage_paddle` (new_id, dam_pad_canopy, dam_status_canopy, dam_pad_date) VALUES ( ?, ?, ?, ?)");
            ...
            if($stmt4){
              ...
              echo "<script>window.location.href='maintenance_paddle_excellent_list.php';</script>";
            }
          }
        }
      }
    ...
    if(!empty($_POST["dam_pad_pole"]))
    {
      foreach($_POST["dam_pad_pole"] as $b)
      {
        include "dbconnection.php";
        ...
        $stmt7 = $conn->prepare("SELECT * FROM `damage_paddle` WHERE new_id=? AND dam_pad_date=?");
        ...
        if($row7){
          $stmt8 = $conn->prepare("UPDATE `damage_paddle` SET dam_pad_pole=?, dam_status_pole=? WHERE new_id=? AND dam_pad_date=?");
          ...
          if($row9){
            $stmt10 = $conn->prepare("INSERT INTO `damage_paddle` (new_id, dam_pad_pole, dam_status_pole, dam_pad_date) VALUES ( ?, ?, ?, ?)");
    ...
    if(!empty($_POST["dam_pad_seat"]))
    {
      foreach($_POST["dam_pad_seat"] as $c)
      {
        include "dbconnection.php";
        ...
        $stmt13 = $conn->prepare("SELECT * FROM `damage_paddle` WHERE new_id=? AND dam_pad_date=?");
        ...
        if($row13){
          $stmt14 = $conn->prepare("UPDATE `damage_paddle` SET dam_pad_seat=?, dam_status_seat=? WHERE new_id=? AND dam_pad_date=?");
          ...
          if($row15){
            $stmt16 = $conn->prepare("INSERT INTO `damage_paddle` (new_id, dam_pad_seat, dam_status_seat, dam_pad_date) VALUES ( ?, ?, ?, ?)");
    ...
    if(!empty($_POST["dam_pad_paddle"]))
    {
      foreach($_POST["dam_pad_paddle"] as $d)
      {
        include "dbconnection.php";
        ...
        $stmt19 = $conn->prepare("SELECT * FROM `damage_paddle` WHERE new_id=? AND dam_pad_date=?");
        ...
        if($row19){
          $stmt20 = $conn->prepare("UPDATE `damage_paddle` SET dam_pad_paddle=?, dam_status_paddle=? WHERE new_id=? AND dam_pad_date=?");
          $stmt20->bind_param("ssss", $dam_paddle, $status_paddle, $d, $dam_date);
          $stmt20->execute();
        }
        else{
          ...
          if($row21){
            $stmt22 = $conn->prepare("INSERT INTO `damage_paddle` (new_id, dam_pad_paddle, dam_status_paddle, dam_pad_date) VALUES ( ?, ?, ?, ?)");
            $stmt22->bind_param("ssss", $d, $dam_paddle, $status_paddle, $dam_date);
            $stmt22->execute();
    ...
    if(!empty($_POST["dam_pad_crack"]))
    {
      foreach($_POST["dam_pad_crack"] as $e)
      {
        include "dbconnection.php";
        ...
        $stmt25 = $conn->prepare("SELECT * FROM `damage_paddle` WHERE new_id=? AND dam_pad_date=?");
        ...
        if($row25){
          $stmt26 = $conn->prepare("UPDATE `damage_paddle` SET dam_pad_crack=?, dam_status_crack=? WHERE new_id=? AND dam_pad_date=?");
          ...
          if($row27){
            $stmt28 = $conn->prepare("INSERT INTO `damage_paddle` (new_id, dam_pad_crack, dam_status_crack, dam_pad_date) VALUES ( ?, ?, ?, ?)");
    ...
    if(!empty($_POST["dam_pad_leak"]))
    {
      foreach($_POST["dam_pad_leak"] as $f)
      {
        include "dbconnection.php";
        ...
        $stmt31 = $conn->prepare("SELECT * FROM `damage_paddle` WHERE new_id=? AND dam_pad_date=?");
        ...
        if($row31){
          $stmt32 = $conn->prepare("UPDATE `damage_paddle` SET dam_pad_leak=?, dam_status_leak=? WHERE new_id=? AND dam_pad_date=?");
          ...
          if($row33){
            $stmt34 = $conn->prepare("INSERT INTO `damage_paddle` (new_id, dam_pad_leak, dam_status_leak, dam_pad_date) VALUES ( ?, ?, ?, ?)");
