  ...
  if(!empty($_SESSION["display_list"]))
  {
    foreach($_SESSION["display_list"] as $keys => $values)
    {
      ...
      <tr>
        ...
        <td align="center">RM '.number_format($values["facilities_rate"]).'<input type="hidden" name="hidden_list_rate'.$count.'" id="list_rate'.$count.'" size="1" value="'.$values["facilities_rate"].'"/></td>
